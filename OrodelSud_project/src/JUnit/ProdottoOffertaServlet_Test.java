...
@WebServlet("/ProdottoOffertaServlet_Test")

public class ProdottoOffertaServlet_Test extends HttpServlet{
	private static final long serialVersionUID = 1L;

	public ProdottoOffertaServlet_Test() {
        super();
    }
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			conteggio = interfaccia.conteggiofferta();
			}
			catch (Exception e) {
		...
		try {
		prodotti = interfaccia.prodottofferta(contatore);
		}
		catch (Exception e) {
			response.sendError(500, "Errore nell'operazione");
			}
		...
		if (contatore==1 && json.equals("")) {
			...
			RequestDispatcher view=request.getRequestDispatcher("Prodottofferta.jsp");
